  ...
  timeout(time: 600, unit: "SECONDS") {
    try {
      if (currentBuild.currentResult == "SUCCESS" && env.GIT_LOCAL_BRANCH ==~ /(master|develop)/) {
        ...
        artifacts.uploadArtifacts(filePaths, sprintf('libiroha/linux/%4$s/%1$s-%2$s-%3$s', [env.GIT_LOCAL_BRANCH, sh(script: 'date "+%Y%m%d"', returnStdout: true).trim(), commit.take(6), platform]))
      }
    }
    finally {
